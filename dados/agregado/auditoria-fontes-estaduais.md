# Auditoria de fontes estaduais

Atualizada em 13 de agosto de 2026. Regra de inclusão: somente fonte oficial `.gov.au`, acessível e arquivável, com granularidade identificável.

## Resultado

- ACT: scores por grupo ANZSCO disponíveis no PDF 2025–26. Totais da rodada de 11 de junho de 2026 incorporados por subclass: 77 convites 190 e 5 convites 491. O PDF oficial 2024–25 ainda aparece indexado, mas o servidor retorna 404; não incorporado.
- NSW: publica critérios, listas de grupos elegíveis e funcionamento das rodadas, mas não resultados por ocupação/score nem totais de cada rodada.
- VIC: publica critérios de seleção, prioridades e alocações, mas não resultados por ocupação/score nem totais de cada rodada.
- QLD: publica listas de elegibilidade, fatores de seleção e alguns avisos de campanhas, mas não uma tabela de resultados por ocupação/score.
- SA: sete publicações oficiais de dezembro de 2025 a maio de 2026 já estão incorporadas por sub-major group e subclass. Não foi localizada série oficial equivalente anterior no site atual.
- TAS: não publica resultados por ocupação. A fonte confirma 1.200 nomeações 190 e 650 nomeações 491 em 2025–26; esses valores não são convites e deixaram de alimentar essa métrica. A página oficial informa alocação 2026–27 de 1.250/800, mas bloqueia arquivamento automatizado via Cloudflare; encontrada, ainda não incorporada.
- WA: resultados por ocupação incorporados para maio, outubro e dezembro de 2025; janeiro, março e maio de 2026. PDFs adicionais indexados para fevereiro/abril e trades de março não estavam acessíveis por URL oficial verificável; não incorporados.
- NT: não foi localizada publicação oficial `.gov.au` de resultados por ocupação, score ou rodada. O antigo endereço de migração redireciona para domínio não governamental e foi rejeitado.

## Fontes oficiais consultadas

- https://www.act.gov.au/migration/resources/canberra-matrix-invitation-round
- https://www.nsw.gov.au/visas-and-migration/skilled-visas/skilled-work-regional-visa-subclass-491
- https://liveinmelbourne.vic.gov.au/migrate/skilled-migration-visas/registration-of-interest-for-victorian-state-visa-nomination
- https://www.migration.qld.gov.au/resources/frequently-asked-questions
- https://migration.sa.gov.au/news?category=invitations-issued
- https://www.migration.tas.gov.au/news/processing_times_and_allocation_usage
- https://www.migration.tas.gov.au/news/2024-25_program_open_to_rois
- https://migration.wa.gov.au/our-services-support/state-nominated-migration-program
- https://immi.homeaffairs.gov.au/visas/working-in-australia/skillselect/invitation-rounds
- https://immi.homeaffairs.gov.au/what-we-do/state-and-territory-nomination-allocations
